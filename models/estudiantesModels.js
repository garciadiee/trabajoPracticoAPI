...
exports.getCursosDelEstudiante = async (id) =>{
    const [rows, field] = await db.execute ('SELECT cursos.nombre, cursos.descripcion FROM cursos INNER JOIN estudiantes ON estudiantes_cursos.estudiante_id = estudiantes.id AND estudiantes.id = ? INNER JOIN cursos ON estudiantes_cursos.curso_id = cursos.id')
}