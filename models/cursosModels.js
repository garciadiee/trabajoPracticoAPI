...
exports.getCursoById = async (id) => {
  const [rows, fields] = await db.execute(
    "SELECT nombre, descripcion FROM cursos WHERE id=?",
    [id]
  );
  console.log(rows);
  return rows;
...
exports.updateCurso = async (curso) => {
  const [rows, fields] = await db.execute(
    ...
    [curso.nombre, curso.descripcion, curso.id]
  );
  return rows;
};

exports.deleteCursoById = async (id) => {
  const [rows, fields] = await db.execute("DELETE FROM cursos WHERE id = ?", [
    id,
  ]);
  return rows;
};
...
exports.addEstudianteAUnCurso = async (estudiante) => {
  const [rows, fields] = await db.execute(
    "INSERT INTO `estudiantes_cursos` VALUE ( ?, ? )",
