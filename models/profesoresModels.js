...
exports.getProfesoresById = async (id) => {
  const [rows, fields] = await db.execute(
    "SELECT nombre, especialidad, email FROM profesores WHERE id=?",
    [id]
  );
  console.log(rows);
  return rows;
...
exports.addProfesor = async (nuevoprofesor) => {
  const [rows, fields] = await db.execute(
    ...
    [nuevoprofesor.nombre, nuevoprofesor.especialidad, nuevoprofesor.email]
  );
  return rows;
};
exports.updateProfesor = async (profesor) => {
  const [rows, fields] = await db.execute(
    "UPDATE profesores SET nombre = ?, especialidad = ?, email = ? WHERE id = ?",
    [profesor.nombre, profesor.especialidad, profesor.email, profesor.id]
  );
  return rows;
};
